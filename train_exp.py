...
from scipy.stats import entropy, expon
...
@ex.automain
def main(_run, validation, num_epochs, batch_size, dataset, repulsive_ensemble, sep_channels, norm_after, eps, multiply_input, input_size, exp_params):
    logger=get_logger()
    if validation:
    ...
    freq_mask = np.empty((input_size[0], input_size[1]))
    diag_weights = np.sqrt(expon.pdf(np.linspace(0.0, exp_params['end'], input_size[0]+input_size[1]-1), 0.0, exp_params['rate']))
    for i in range(input_size[0]):
        freq_mask[i, :] = diag_weights[i:i+input_size[1]]
    freq_mask = jnp.array(freq_mask[..., None])
    def loss_fn(params, state, bx, by, epoch):
        ...
        if repulsive_ensemble:
            ...
            jacobian = jax.vmap(jax.vmap(jax.grad(__per_example_apply, 0), (0, 0, None, None), 0), (None, None, 0, 0), 0)(bx, by, params, new_state) # [n_members, batch_size, *x.size]
            if sep_channels:
                axes = (2, 3)
            else:
                axes = (2, 3, 4)
