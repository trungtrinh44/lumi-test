...
def my_config():
    ...
    test_batch_size = 512
    n_members = 4  # Number of components in the posterior
...
@ex.automain
def main(_run, validation, num_epochs, batch_size, dataset, repulsive_ensemble, diag, sep_channels, norm_after, eps, multiply_input, input_size):
    logger=get_logger()
    if validation:
    ...
    freq_mask = np.zeros(input_size)
    for m in range(sum(input_size)-1-diag):
        for i in range(input_size[0]):
            for j in range(input_size[1]):
                for k in range(min(input_size[2], m-i-j)):
                    freq_mask[i,j,k]=1.0
    freq_mask = jnp.array(freq_mask)
